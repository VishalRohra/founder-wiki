...
- **Images — selective and purposeful.** Include images only when they convey information that text cannot (diagrams, charts, data visualizations, product screenshots showing a specific UI pattern). Do not include every image from a source — curate. If you can say it in text, say it in text. An image earns its place when it makes a concept immediately graspable in a way words cannot. For a pitch deck article, include 3-5 key slide examples that illustrate the principles — not all 26 slides from the source.
...
- **Images must be curated, not dumped.** Transfer relevant images from raw to wiki, but be selective. Include 3-5 images per article maximum. Choose images that convey information text cannot: diagrams, charts, data visualizations, before/after comparisons. Skip decorative images, duplicate illustrations of the same point, and low-information screenshots. The viewer renders images in Wikipedia-style float thumbnails — too many images makes the article unreadable.
